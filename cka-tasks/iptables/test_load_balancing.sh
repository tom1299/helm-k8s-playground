...
for i in {1..6}; do
    timeout 1 docker exec client sh -c "echo \"Load balance message $i\" | nc -u -w0 192.168.1.1 27019"
done

sleep 0.2

# Check if messages were distributed across backends
BACKEND1_COUNT=$(docker logs backend | grep -c "Load balance message")
BACKEND2_COUNT=$(docker logs backend2 | grep -c "Load balance message") 
BACKEND3_COUNT=$(docker logs backend3 | grep -c "Load balance message")
