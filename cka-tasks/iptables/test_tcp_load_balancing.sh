...
for i in {1..6}; do
    timeout 1 docker exec client sh -c "echo \"Tcp load balance message $i\" | nc -t -w0 192.168.1.1 27020"
done

sleep 0.2

# Check if messages were distributed across backends
BACKEND1_COUNT=$(docker logs backend | grep -c "Tcp load balance message")
BACKEND2_COUNT=$(docker logs backend2 | grep -c "Tcp load balance message") 
BACKEND3_COUNT=$(docker logs backend3 | grep -c "Tcp load balance message")
